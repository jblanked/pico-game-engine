...
static void player_render(Entity *self, Draw *draw, Game *game)
{
    ...
    if (player_lives <= 0)
    {
        ...
        draw->text(Vector(135, 120), "Game Over", 1, game->fg_color);
        return;
    }
    ...
    draw->tft.drawBitmap(player_x, player_y, player_bitmap, PLAYER_WIDTH, PLAYER_HEIGHT, TFT_DARKCYAN);
    ...
    for (int i = 0; i < enemy_count; ++i)
    {
        ...
        draw->tft.drawBitmap(enemy_x[i], enemy_y[i], enemy_bitmap, 9, 8, TFT_RED);
    }
    ...
    for (int i = 0; i < MAX_BULLETS; ++i)
    {
        if (bullet_active[i])
        {
            ...
            draw->tft.fillRect(bullet_center_x, bullet_center_y, BULLET_WIDTH, BULLET_HEIGHT, TFT_BLACK);
        }
    ...
    for (int i = 0; i < player_lives; ++i)
    {
        ...
        draw->tft.drawBitmap(SCREEN_WIDTH - (i + 1) * 12, 8, life_icon, 7, 7, TFT_BLACK);
    }
    ...
    if (bonus_life_active)
    {
        ...
        draw->tft.drawBitmap(bonus_life_x, bonus_life_y, bonus_life_icon, 7, 7, TFT_BLACK);
    }
